...
def task2_a():
    ...
    def Exercise_a():
        ...
        I = imread('Week1/Images/trui.png')
...
def task2_b():
    ...
    def convolvefft(image, kernel):
        def _pad_kernel():
            ...
            return np.fft.ifftshift(np.pad(kernel, (((sz[0] + 1) // 2, sz[0] // 2),((sz[1] + 1) // 2, sz[1] // 2)),'constant'))

        padded_kernel = _pad_kernel()
        image_fft = np.fft.fft2(image)
        kernel_fft = np.fft.fft2(padded_kernel)
        conv_fft = image_fft * kernel_fft
        conv = np.absolute(np.real(np.fft.ifft2(conv_fft)))

        return conv

    image = np.array(imread("Week1/Images/trui.png"))
    ks = [3, 7, 13, 21]
    n = 1

    fig, axs = plt.subplots(len(ks), 4, figsize=(24, 24))
...
def task2_c():
    ...
    I = img_as_float(imread("Week1/Images/cameraman.tif"))
    I_1 = wave(I, 1, 1, 0)
    I_2 = wave(I, 1, 0, 1)
    I_3 = wave(I, 1, 1, 1)
    I_cos = [I_1, I_2, I_3]
